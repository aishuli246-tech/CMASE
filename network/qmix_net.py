...
from network.embedding_net import EmbeddingNet
from network.attention_net import AttentionNet
...
class QMixNet(nn.Module):
    def __init__(self, args):
        ...
        self.embedding_net = EmbeddingNet(args)
        self.attention_net = AttentionNet(args)

    def forward(self, obs, u, states, q_values): 
        # obs的shape为(episode_num, max_episode_len, n_agents,  obs_dim)
        # u的shape为(episode_num, max_episode_len，n_agents, action_dim)
        # states的shape为(episode_num, max_episode_len，state_shape)
        # q_values的shape为(episode_num, max_episode_len，n_agents)
        ve = self.embedding_net(obs, u)    #(episode_num*max_episode_len, n_agents, embedding_hidden_dim)
        nse = self.get_nse(q_values.reshape(-1, self.args.n_agents))
        nse_weighted = self.attention_net(ve, states.reshape(-1, self.args.state_shape), nse) #(episode_num * max_episode_len，n_agents)
        ...
        g = torch.cat([states, nse_weighted], dim=-1)
        w1 = torch.abs(self.hyper_w1(g))  # (1920, 160)
        b1 = self.hyper_b1(g)  # (1920, 32)
        ...
        w2 = torch.abs(self.hyper_w2(g))  # (1920, 32)
        b2 = self.hyper_b2(g)  # (1920, 1)
    ...
    def get_nse(self, q_values):
        total_sum = q_values.sum(dim=-1, keepdim=True)
        nse = total_sum - q_values
        return nse
